...
from PySide6.QtCore import QEvent, QPoint, QPointF, QSize, QUrl, Qt
from PySide6.QtGui import QHelpEvent, QImage, QPalette, QTextCursor, QWheelEvent
...
class UITests(unittest.TestCase):
    ...
    def test_streaming_scroll_tracks_visible_text_and_respects_wheel(self) -> None:
        view = ChatView()
        view.resize(900, 500)
        view.show()
        bubble = view.add_streaming("模型 · 标准模式", False)
        ...
        wheel = QWheelEvent(
            QPointF(30, 30),
            QPointF(view.viewport().mapToGlobal(QPoint(30, 30))),
            QPoint(0, 0),
            QPoint(0, 120),
        ...
        self.assertFalse(view.follows_output)
        paused = view.verticalScrollBar().value()
        QTest.qWait(80)
        self.assertLessEqual(view.verticalScrollBar().value(), paused)

        view.verticalScrollBar().setValue(0)
        view.resume_follow()
        self.assertEqual(view.verticalScrollBar().value(), 0)
        self.assertTrue(self.wait_for(lambda: view.verticalScrollBar().value() > 0))
        self.assertLess(view.verticalScrollBar().value(), view.verticalScrollBar().maximum())
        self.assertTrue(
            self.wait_for(
                lambda: view.verticalScrollBar().value()
    ...
    def test_short_user_message_bubble_uses_its_natural_width(self) -> None:
        ...
        self.assertEqual(short.text_view.document().begin().layout().lineCount(), 1)

        long = view.add_user("很长的提问内容" * 100)
        self.app.processEvents()
        self.assertEqual(long.card.width(), min(MESSAGE_CONTENT_MAX_WIDTH + 28, long.width()))
        self.assertEqual(long.text_view.width(), long.card.width() - 28)
        view.close()

    def test_composer_and_both_message_roles_share_content_width(self) -> None:
        cfg = dict(DEFAULTS)
        ...
        user = window.chat_view.add_user("较长的用户输入" * 200)
        assistant = window.chat_view.add_assistant(
            "较长的模型回答" * 200, thinking=False
        )
        self.app.processEvents()
        self.assertEqual(window.chat_input.viewport().width(), MESSAGE_CONTENT_MAX_WIDTH)
        self.assertEqual(assistant.content_view.width(), MESSAGE_CONTENT_MAX_WIDTH)
        self.assertEqual(user.text_view.width(), MESSAGE_CONTENT_MAX_WIDTH)
        window.close()
    ...
    def test_chat_input_scrolls_at_fixed_heights_and_keeps_expansion(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        window = MainWindow(cfg, MemoryStore(), self.app)
        ...
        editor = window.chat_input
        button = window.input_panel.expand_button
        expansion_heights: list[int] = []
        editor.heightChanged.connect(lambda: expansion_heights.append(editor.height()))
        self.assertEqual(editor._height_animator.timer.interval(), 16)
        self.assertEqual(
            editor._height_animator.timer.timerType(), Qt.TimerType.PreciseTimer
        )
        scrollbar = editor.verticalScrollBar()
        collapsed_panel_height = window.input_panel.height()
        self.assertEqual(editor.height(), ChatTextEdit.COLLAPSED_HEIGHT)
        self.assertFalse(button.isChecked())

        editor.setPlainText("超长输入" * 300)
        editor.moveCursor(QTextCursor.MoveOperation.End)
        self.app.processEvents()
        self.assertGreater(scrollbar.maximum(), 0)
        self.assertGreaterEqual(
            scrollbar.value(), scrollbar.maximum() - editor.fontMetrics().height()
        )
        self.assertEqual(editor.height(), ChatTextEdit.COLLAPSED_HEIGHT)
        self.assertEqual(window.input_panel.height(), collapsed_panel_height)
        ...
        self.assertTrue(editor.is_expanded())
        self.assertTrue(
            self.wait_for(lambda: editor.height() == ChatTextEdit.EXPANDED_HEIGHT)
        )
        self.assertTrue(
            self.wait_for(
                lambda: window.input_panel.height() - collapsed_panel_height
                == ChatTextEdit.EXPANDED_HEIGHT - ChatTextEdit.COLLAPSED_HEIGHT
            )
        )
        expanded_panel_height = window.input_panel.height()
        self.assertEqual(editor.height(), ChatTextEdit.COLLAPSED_HEIGHT * 3)
        self.assertGreaterEqual(len(expansion_heights), 8)
        self.assertEqual(expansion_heights, sorted(expansion_heights))
        self.assertGreater(expansion_heights[0], ChatTextEdit.COLLAPSED_HEIGHT)
        self.assertEqual(
            expanded_panel_height - collapsed_panel_height,
            ChatTextEdit.EXPANDED_HEIGHT - ChatTextEdit.COLLAPSED_HEIGHT,
        )
        self.assertEqual(
            window.chat_view.viewportMargins().bottom(), expanded_panel_height
        )
        ...
        self.assertEqual(window.input_panel.height(), expanded_panel_height)

        editor.clear()
        window.on_new_chat()
        self.app.processEvents()
        self.assertEqual(scrollbar.maximum(), 0)
        self.assertEqual(editor.height(), ChatTextEdit.EXPANDED_HEIGHT)
        self.assertEqual(window.input_panel.height(), expanded_panel_height)
        self.assertTrue(button.isChecked())
        self.assertLessEqual(
            window.input_panel.geometry().bottom(), window.chat_workspace.height()
        )

        button.click()
        self.assertTrue(
            self.wait_for(lambda: editor.height() == ChatTextEdit.COLLAPSED_HEIGHT)
        )
        self.assertTrue(
            self.wait_for(lambda: window.input_panel.height() == collapsed_panel_height)
        )
        self.assertEqual(editor.height(), ChatTextEdit.COLLAPSED_HEIGHT)
        self.assertEqual(window.input_panel.height(), collapsed_panel_height)
        window.close()

    def test_expanded_welcome_intro_stays_above_composer_at_minimum_size(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        window = MainWindow(cfg, MemoryStore(), self.app)
        ...
        window.show()
        window.input_panel.expand_button.click()
        self.assertTrue(
            self.wait_for(
                lambda: window.chat_input.height() == ChatTextEdit.EXPANDED_HEIGHT
            )
        )
        self.assertFalse(window.welcome_suggestions.isVisible())
        subtitle = window.welcome.findChild(QWidget, "subText")
        self.assertIsNotNone(subtitle)
        subtitle_bottom = subtitle.mapTo(
            window.chat_workspace, QPoint(0, subtitle.height())
        ).y()
        card_top = window.input_panel.card.mapTo(
            window.chat_workspace, QPoint(0, 0)
        ).y()
        self.assertLess(subtitle_bottom, card_top)

        window.input_panel.expand_button.click()
        self.assertTrue(
            self.wait_for(
                lambda: window.chat_input.height() == ChatTextEdit.COLLAPSED_HEIGHT
            )
        )
        self.assertTrue(window.welcome_suggestions.isVisible())
        window.close()
    ...
    def test_harness_warm_start_only_builds_webengine_after_switch(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        window = MainWindow(cfg, MemoryStore(), self.app)
        ...
        with (
            ...
            patch.object(page.surface, "show_status") as show_status,
            patch.object(page.surface, "set_detail") as set_detail,
        ):
            page.warm()
            runtime_start.assert_called_once()
            page.runtime.stateChanged.emit("正在启动")
            page.runtime.failed.emit("启动失败")
            page.runtime._ready = True
            page.runtime._url = url
            page.runtime.ready.emit(url)
            show_web.assert_not_called()
            show_status.assert_not_called()
            set_detail.assert_not_called()
            self.assertIs(window.page_stack.currentWidget(), window.chat_page)
            self.assertIsNone(page.surface._web_view)
            ...
            self.app.processEvents()
            show_web.assert_not_called()
            window._switch_mode("harness")
            show_web.assert_called_once_with(url)
            runtime_start.assert_called_once()
        window.close()

    def test_harness_ready_draws_only_while_its_page_is_selected(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        window = MainWindow(cfg, MemoryStore(), self.app)
        ...
        with (
            ...
            page.runtime._url = url
            window._switch_mode("harness")
            self.assertEqual(show_web.call_count, 2)
            show_web.assert_called_with(url)
        window.close()

    def test_harness_ready_while_minimized_draws_after_restore(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        window = MainWindow(cfg, MemoryStore(), self.app)
        ...
        with (
            ...
            page.runtime._url = url
            page.runtime.ready.emit(url)
            self.assertTrue(page._surface_pending)
            show_web.assert_not_called()
            self.assertTrue(window.isMinimized())
            ...
            self.app.processEvents()
            show_web.assert_called_once_with(url)
            self.assertFalse(page._surface_pending)
        window.close()

    def test_harness_selector_returns_from_settings_before_drawing(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        window = MainWindow(cfg, MemoryStore(), self.app)
        ...
        with (
            ...
            page.runtime._ready = True
            page.runtime._url = "http://127.0.0.1:8123/ui?token=abc"

            window._switch_mode("harness")
            self.assertIs(window.page_stack.currentWidget(), page)
            show_web.assert_called_once_with(page.runtime.url)
        window.close()
    ...
    def test_completed_round_starts_full_context_title_summary(self) -> None:
        ...
        window._streams[conversation["id"]] = context
        with patch.object(window, "_start_title_summary") as summarize:
            window._on_done(context)
    ...
    def test_parallel_conversations_keep_streams_and_drafts_when_switching(self) -> None:
        cfg = dict(DEFAULTS)
        cfg["models"] = list(DEFAULTS["models"])
        cfg["api_key"] = "test"
        cfg["deep_thinking"] = False
        store = MemoryStore()
        window = MainWindow(cfg, store, self.app)
        gates = {"问题 A": threading.Event(), "问题 B": threading.Event()}

        def stream_chat(*args, **kwargs):
            label = args[4][-1]["content"]
            yield "content", f"{label} 的开头"
            while not gates[label].wait(0.01):
                if args[7].is_set():
                    return
            yield "content", f"，{label} 的结尾"

        try:
            with (
                patch("app.worker.api.stream_chat", side_effect=stream_chat),
                patch.object(window, "_start_title_summary"),
            ):
                window.chat_input.setPlainText("问题 A")
                window.on_send()
                first_id = window._current_conversation_id
                self.assertTrue(self.wait_for(lambda: window._streams[first_id].content))
                window.chat_input.setPlainText("A 的下一轮草稿")
                cursor = window.chat_input.textCursor()
                cursor.setPosition(2)
                window.chat_input.setTextCursor(cursor)
                staged_image = str(ASSETS_DIR / "app-icon.png")
                window.input_panel.image_strip.add_path(staged_image)
                self.assertFalse(window.chat_input.isReadOnly())
                self.assertTrue(
                    window.sidebar._items[first_id].running_indicator._timer.isActive()
                )

                window.on_new_chat()
                self.assertIn(first_id, window._streams)
                window.chat_input.setPlainText("问题 B")
                window.on_send()
                second_id = window._current_conversation_id
                self.assertTrue(self.wait_for(lambda: window._streams[second_id].content))
                self.assertEqual(set(window._streams), {first_id, second_id})
                window.chat_input.setPlainText("B 的下一轮草稿")

                window.on_select_conversation(first_id)
                self.assertEqual(window.chat_input.toPlainText(), "A 的下一轮草稿")
                self.assertEqual(window.chat_input.textCursor().position(), 2)
                self.assertEqual(window.input_panel.image_strip.paths(), [staged_image])
                self.assertEqual(set(window._streams), {first_id, second_id})
                self.assertTrue(
                    window.sidebar._items[second_id].running_indicator._timer.isActive()
                )
                gates["问题 B"].set()
                self.assertTrue(self.wait_for(lambda: second_id not in window._streams))
                self.assertIn(first_id, window._streams)
                self.assertEqual(store.get(second_id)["messages"][-1]["role"], "assistant")

                gates["问题 A"].set()
                self.assertTrue(self.wait_for(lambda: first_id not in window._streams))
                self.assertEqual(window.chat_input.toPlainText(), "A 的下一轮草稿")
                window.on_select_conversation(second_id)
                self.assertEqual(window.chat_input.toPlainText(), "B 的下一轮草稿")
                self.assertEqual(window.input_panel.image_strip.paths(), [])
                self.assertIn("问题 B 的结尾", store.get(second_id)["messages"][-1]["content"])
        finally:
